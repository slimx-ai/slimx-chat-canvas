...
const nodes = new Map();
let selectedNodeId = null, selectedDirection = 'south', focusMode = false, contextMode = 'focused';
...
function findBranchOrigin(nodeId){
  const root = [...nodes.values()].find((n) => !n.parentId);
  const path = contextPath(nodeId);
  if (path.length < 2) return nodes.get(nodeId);
  return path.find((n) => n.parentId === root?.id) || path[1];
}

function focusedContext(nodeId){
  const path = contextPath(nodeId);
  if (path.length <= 2) return path;
  const origin = findBranchOrigin(nodeId);
  const idx = path.findIndex((n) => n.id === origin.id);
  return [path[0], ...path.slice(Math.max(1, idx))];
}
...
window.onkeydown=(e)=>{const map={ArrowUp:'north',ArrowRight:'east',ArrowLeft:'west',ArrowDown:'south'}; if((e.metaKey||e.ctrlKey)&&map[e.key]){selectedDirection=map[e.key]; updateBranchUI();} if(e.key==='/'&&document.activeElement!==el('searchInput')){e.preventDefault();el('searchInput').focus();} if(e.key.toLowerCase()==='m'){contextMode=contextMode==='focused'?'ancestor':'focused'; updateInspector();} if(e.key==='Escape'){el('prompt').blur();}};
